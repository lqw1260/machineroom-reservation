    ...
    <div>
      <el-card class="box-card" shadow="always">
        <div>
          ...
          <p>
            共 <span>{{ tableData.length }}</span> 条申请，剩余
            <span>{{ numOfNotProcesse }}</span> 条申请未处理
          </p>
        </div>
        <div class="record-table">
          <template>
            ...
            >
              ...
              <el-table-column
                v-for="record in recordTableColumn.slice(1, -2)"
                :key="record.prop"
                :prop="record.prop"
                :label="record.label"
                table-layout="auto"
      ...
      <el-dialog title="申请理由" :visible.sync="dialogVisible" width="30%">
        ...
        <span
          slot="footer"
          class="dialog-footer"
          v-if="currentApplication.status === '待审核'"
        >
...
export default {
  ...
  methods: {
    ...
    async agreeApplication() {
      ...
      // }
      let result = await this.$request.put(
        "api/admin-service/acceptApplication/" + this.currentApplication.id
      );
    ...
    async rejectApplication() {
      ...
      // }
      let result = await this.$request.put(
        "api/admin-service/rejectApplication/" + this.currentApplication.id
      );
      if (result.code && result.code === 200) {
        this.$message({
          message: "已驳回申请",
