<template lang="">
  <div>
    ...
    <el-card class="box-card reserve">
        ...
        <el-form :inline="true" :model="formInline" class="demo-form-inline">
          ...
          <el-form-item label="机房类型">
            <el-select
              ...
              style="width: 180px"
            >
              <el-option label="类型A10" value="A10"></el-option>
              <el-option label="类型A11" value="A11"></el-option>
              <el-option label="类型A12" value="A12"></el-option>
              <el-option label="类型A20" value="A20"></el-option>
              <el-option label="类型A21" value="A21"></el-option>
              <el-option label="类型A22" value="A22"></el-option>
            </el-select>
          </el-form-item>
      ...
      <el-card class="box-card">
        ...
        <div class="machineroomCard">
          <div
            v-for="item in machineroom"
            :key="item.machineroomId"
          ...
          >
            <el-card :body-style="{ padding: '0px' }">
              ...
              <div style="padding: 14px">
                <span>{{ item.name }}</span>
                <span style="float: right" :class="item.firstFeature">{{
                  features[item.features[1]]
                }}</span>
                <span style="float: right" :class="item.secondFeature">{{
                ...
                <span
                  style="float: right"
                  class="machineroomFeature clickReserve"
                  @click="clickReserve(item)"
                  >点击预约</span
                >
              </div>
            </el-card>
    ...
    <el-dialog title="提示" :visible.sync="dialogVisible" width="30%">
      ...
      <el-card :body-style="{ padding: '0px' }">
        ...
        <div style="padding: 14px">
          ...
          <span style="float: right" :class="quickRoom.firstFeature">{{
            features[quickRoom.features[1]]
          }}</span>
          <span style="float: right" :class="quickRoom.secondFeature">{{
            features[quickRoom.features[2]]
          }}</span>
          <br />
    ...
    <el-dialog title="提示" :visible.sync="reserveDialog" width="30%">
      <el-form ref="form" :model="reserveRoom" label-width="80px">
        ...
        <el-form-item label="起始节数">
          <el-select
            v-model="reserveRoom.beginSection"
            placeholder="第节"
            style="width: 100px"
          >
            <el-option label="第一节" value="1"></el-option>
        ...
        <el-form-item label="结束节数">
          <el-select
            v-model="reserveRoom.endSection"
            placeholder="第一节"
            style="width: 100px"
          >
            <el-option label="第一节" value="1"></el-option>
        ...
        <el-form-item label="备注">
          <el-input type="textarea" v-model="reserveRoom.remark"></el-input>
        </el-form-item>
      </el-form>
      <span slot="footer" class="dialog-footer">
  ...
  methods: {
    ...
    async getMachineroomInfo() {
      let result = await this.$request.post(
        "api/common-service/queryMachineRoomListByCond"
      );
      if (result.data) {
    ...
    async reserveMachineroom(room) {
      ...
      let result = await this.$request.post(
        "api/user-service/applyMachineRoom",
        m_room
      );
